...
def calc_zw(
    target_sample: samples.galaxy.HaloSample, 
    reference_sample: samples.galaxy.HaloSample, 
    bins: int = 24
) -> samples.galaxy.HaloSample:
    zwgt = ZWeighting(reference_sample.data['z_cor'], bins=bins)
    zs = target_sample.data['z_cor']

    # Update weights in the target sample
    new_data = target_sample.data.copy()
    new_data['weight'] = zwgt.weights_of(zs)

    weighted_sample = samples.galaxy.HaloSample(new_data, verbose=target_sample.verbose)
